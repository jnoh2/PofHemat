# File name: 4a_2-D_pca_adult_aHSC.py
...
def plot_PCA(pca, ratios, cells, unique_cells):
	...
	dot_size = 35
	ax.scatter(pca[:, 0], -pca[:, 1], s = dot_size, c = colors) # PC 2 is flipped for visual alignment with other PC plots
	plt.show()
...
def save_PCA(component_vec, genes):
	for i in range(len(component_vec)):
		...
		if i == 1:
			component = - component # Matches the flip in PC 2 that took place for visual alignment
