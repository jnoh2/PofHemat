# File name: 19_final_graph_mRNA_prot_zoomed.py
...
# protein and mRNA for all cell types. Identical to one produced in
# 18_final_graph_mRNA_prot_all.py, but without the gene specific
# data and mean and median lines drawn. Also calculates the spearman
# correlation between protein and mRNA per cell type and records
# in a file
...
# Given the normalized data of genes expressed in protein and mRNA of all cell types,
# calculate the spearman correlation per cell type between protein and mRNA.
# Write the resulting rho and p-values into a new file.
def record_spearman(data, mRNA_data):
	w3 = open(analysis_dir + write_cell_dir + '19_spearman_corr.csv', 'w+')

	w3.write(',Spearman\n')
	for i in range(len(cell_order)):
		cell_prot_data = data[:, i]
		cell_mRNA_data = mRNA_data[:, i]
		# Determines genes expressed in both protein and mRNA; should be redundant here
		both_expressed = (((cell_prot_data) > 0).astype(int) * ((cell_mRNA_data) > 0).astype(int)).astype(bool)
		# Calculates the spearman correlation test
		spear = spearmanr(cell_mRNA_data[both_expressed], cell_prot_data[both_expressed])
		# Writes into the file
		w3.write(cell_order[i] + ',' + str(spear[0]) + ',' + str(spear[1]) + '\n')
...
def plot_data(bp_gatherings, mean_points, median_points):
	...
	ax.plot(np.arange(1, 5, 1), mean_points, '--', c = 'black', alpha = 1.0, label = 'mean')
	ax.plot(np.arange(1, 5, 1), median_points, 'k', c= 'black', label = 'median')
	...
	# Create legend for the mean and median lines
	x_shift = 0.68 # Increase -> 
	y_shift = 0.53 # Increase -> 
	font_size = 11.75
	fig.legend(loc = 'center left', bbox_to_anchor = (x_shift, y_shift), 
		frameon = False, fontsize = font_size)

	# Save the figure
	plt.savefig(analysis_dir + write_cell_dir + '19_mRNA_prot_zoomed')

# Main function
def main():
	# Starting message
	print('Starting [19_final_graph_mRNA_prot_zoomed]\n'); flush()
	...
	record_spearman(data, mRNA_data)
	...
	# Keep track of their means and medians
	bp_gatherings = []
	mean_points = []
	median_points = []
	for i in range(len(cell_order)):
	...
	# Plot the overall fold change values, indicating their means and medians
	plot_data(bp_gatherings, mean_points, median_points)
	
	# Closing message
	print('\n[19_final_graph_mRNA_prot_zoomed] complete')
