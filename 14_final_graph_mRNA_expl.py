# File name: 14_final_graph_mRNA_expl.py
...
#
# Description: 
# For each cell type, find the fold change differences in protein vs mRNA
# against every other cell type. Plot the fold change differences as a
# boxplot, clearly indicating the mean and median.
...
# Given the fold change difference values, plot in a boxplot, clearly
# indicating the mean and median.
def plot_statistics(bp_gatherings, mean_points, median_points):
	...
	# Draw line graph against mean and median values; mean = purple
	ax.plot(np.arange(1, 5, 1), mean_points, 'k', c = 'purple')
	ax.plot(np.arange(1, 5, 1), median_points, 'k')

	# Color code the various aspects of boxplot
	...
	# Save the plot
	plt.savefig(analysis_dir + write_cell_dir + '14_mRNA_fold_diffs')

# Main function
def main():
	# Starting message
	print('Starting [14_final_graph_mRNA_expl]\n'); flush()
	...
	for i in range(len(cell_order)): # For each cell type (against every other cell type)
	...
	# Plot the statistics found
	plot_statistics(bp_gatherings, mean_points, median_points)
	
	# Closing message
	print('\n[14_final_graph_mRNA_expl] complete')
